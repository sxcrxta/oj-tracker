...
- **코드 정리** (`/library.html`): 한 문제에 풀이를 여러 개 저장한다 (예: 바텀업 DP와 탑다운 DP).
  맞은 제출에서 코드를 바로 가져오거나 붙여넣고, 이름·핵심 아이디어·시간복잡도·메모를 적는다.
  **"Claude가 설명 채우기"** 버튼을 누르면 워커가 이름과 설명을 제안한다 (`_shared/solutions.js`).
- **문제 연결**: 문제끼리 확장 버전 / 기본 버전 / 비슷한 문제로 잇는다. 한쪽에서 연결하면 반대쪽에서도 방향을 뒤집어 보여준다.
  dshs.app 문제와 연습장 문제를 서로 이을 수 있다.
- **연결 지도** (`/graph.html`): 문제를 점, 연결을 선으로 그린다 (직접 만든 힘 기반 배치, canvas).
  점을 끌어 옮기고, 빈 곳을 끌면 화면이 움직이고, 휠로 확대·축소한다. 점을 누르면 그 문제의 연결과 바로가기가 나온다.
- 세 곳(대시보드 문제 창, 연습장 문제 페이지, dshs.app 힌트 패널)에서 모두 열 수 있다.
